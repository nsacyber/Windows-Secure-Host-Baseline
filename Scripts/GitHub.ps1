...
"@
    ...
    Process {
        ...
        # -Method Post -Uri 'https://api.github.com/markdown/raw' -ContentType 'text/plain' -Body $requestBody
        ...
        if ($statusCode -eq 200) {
            ...
            $html = $htmlTemplate -f $returnedHtml # this throws an error 'input string was not in a correct format' if brackets are not escaped in the template
        } else {
            throw 'Conversion failed with status code $statusCode'
        }
...
Function Convert-AllMarkdownToHtml() {
    [CmdletBinding()] 
    [OutputType([void])]
    Param(
    ...
    Process {
        $markdownFiles = [System.IO.FileInfo[]]@(Get-ChildItem -Path $Path -Recurse -Include "*.md")

        # don't convert empty files
        $markdownFiles | Where {$_.Length -gt 0 } | ForEach {
            ...
            $html = Get-GitHubHtmlFromRawMarkdown -Markdown $markdown
